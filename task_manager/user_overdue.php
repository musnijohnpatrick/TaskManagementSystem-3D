...
$tasks = $conn->query("SELECT * FROM tasks WHERE assigned_to=$user_id AND due_date < '$today' AND status!='Completed'");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="sidebar">
  ...
  <!-- Main Content -->
  <div class="content">
    <h2>⏰ Overdue Tasks - <?php echo $_SESSION['user']['username']; ?></h2>
    <hr>
      ...
      <tbody>
        <?php if ($tasks->num_rows > 0) { while($row = $tasks->fetch_assoc()) { ?>
          <tr>
            ...
            <td class="text-danger fw-bold"><?php echo $row['due_date']; ?> (Overdue)</td>
            <td><span class="badge bg-danger"><?php echo $row['status']; ?></span></td>
          </tr>
        <?php }} else { ?>
          <tr><td colspan="4" class="text-center text-muted">No overdue tasks.</td></tr>
        <?php } ?>
      </tbody>
    </table>
  </div>
